...
If a pull from `origin/main` or a project regeneration resets any signed target to `com.noopapp.*`, change it back to the matching `com.mumer.*` identifier before running `xcodegen generate`, building, or installing on the phone. Use `.claude/skills/update-and-install-device` for this workflow when running Claude.
