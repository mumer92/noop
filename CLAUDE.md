...
## Local physical-device build identity
...
If an upstream update or a project regeneration resets any signed target to `com.noopapp.*`, change it back to the matching `com.mumer.*` identifier before running `xcodegen generate`, building, or installing on the phone. Use the repo-local `update-and-install-device` skill for this workflow.
